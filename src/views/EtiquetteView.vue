...
<template>
  <div class="py-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
    ...
    <template v-else-if="items.length > 0">
      ...
      <router-link v-for="item in filtered" :key="item.id" :to="`/etiquette/${item.id}`" class="group p-6 rounded-3xl bg-[var(--surface)] border border-[var(--border-default)] hover:border-amber-500 hover:shadow-xl transition-all space-y-4">
